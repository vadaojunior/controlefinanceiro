...
export const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (categoryToEdit) {
      editCategory(categoryToEdit.id, { name: name.trim(), type, color });
    } else {
      addCategory({ name: name.trim(), type, color, icon: 'Grid' });
    }
    onClose();
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-end space-x-2 pt-3 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-xl text-xs font-semibold bg-amber-600 hover:bg-amber-500 text-white shadow-md"
            >
              {categoryToEdit ? 'Salvar' : 'Cadastrar'}
            </button>
          </div>
        </form>
      </div>
